...
console.log("=== INTACT FRESHLY RESTORED CODE COUNTS ===");
console.log("Total length of arrays spread in QUESTIONS_RAW:", arrays.length);

const bancaCounts: Record<string, number> = {};
const cycleCounts: Record<string, number> = {};

arrays.forEach((q: any) => {
...
console.log("Bancas found:", bancaCounts);
console.log("Cycles found:", cycleCounts);

const residencyCount = arrays.filter((q: any) => q.banca && q.banca !== 'Trilha Estudante').length;
console.log("Total Residência questions (has banca and not 'Trilha Estudante'):", residencyCount);

const studentCount = arrays.filter((q: any) => !q.banca || q.banca === 'Trilha Estudante').length;
console.log("Total Estudante questions (no banca or is 'Trilha Estudante'):", studentCount);
